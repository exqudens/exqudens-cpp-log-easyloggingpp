...
class ConanConfiguration(ConanFile):
    ...
    def requirements(self):
        try:
            ...
            self.requires("easyloggingpp/9.89", visible=False, build=True)
        except Exception as e:
            self.output.error(e)
            raise e
    ...
    def generate(self):
        try:
            ...
            for dep in self.dependencies.values():
                ...
                if dep.ref.name == "easyloggingpp":
                    copy(self, pattern="*.*", src=Path(dep.package_folder).joinpath("include").as_posix(), dst=Path(self.build_folder).joinpath("easyloggingpp", "include").as_posix())
        except Exception as e:
            self.output.error(e)
            raise e
